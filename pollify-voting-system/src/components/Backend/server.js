...
app.post('/api/create-poll', async (req, res) => {
  try {
    ...
    const poll = new Poll({
      ...
      totalVotes:0 // Initialize totalVotes as 0
    });
...
app.post('/api/polls/:pollId/vote', async (req, res) => {
  ...
  try {
    ...
    poll.totalVotes += 1; // Increment the totalVotes field by 1
...
// Middleware to calculate totalVotes for a poll
const calculateTotalVotes = async (req, res, next) => {
  try {
    const pollId = req.params.pollId;
    ...
    // Iterate over each optionVotes array and sum the vote counts
    poll.optionsVotes.forEach((optionVotes) => {
      optionVotes.forEach((voteCount) => {
        totalVotes += voteCount;
      });
    });

    req.pollTotalVotes = totalVotes;

    next();
  } catch (error) {
    console.error('An error occurred while calculating totalVotes');
    console.error(error);
    res.status(500).json({ error: 'Failed to calculate totalVotes' });
  }
};


// API endpoint to fetch poll and totalVotes
app.get('/api/polls/:pollId/votes', calculateTotalVotes, async (req, res) => {
  try {
    const pollId = req.params.pollId;
    ...
    // Combine options with their respective vote counts
    const optionsWithVotes = options.map((option, index) => ({
      option,
      votes: poll.optionsVotes[index],
    }));
    ...
    res.status(200).json({
      ...
      expiryDate: poll.expiryDate,
      user: poll.user,
      totalVotes: req.pollTotalVotes, // Accessing totalVotes from the middleware
    });
  } catch (error) {
